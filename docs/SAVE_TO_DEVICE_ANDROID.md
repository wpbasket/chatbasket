...
```typescript
const result = await SaveToDevice.save(
...
```typescript
const info = await SaveToDevice.checkFileExists(filePath);
...
```typescript
const info = await SaveToDevice.getFileInfo(contentUri);
...
```typescript
const deleted = await SaveToDevice.deleteFile(contentUri);
// Returns: boolean
```
...
```typescript
const subscription = SaveToDevice.addListener(
  'onSaveComplete',
  (event) => {
...
import SaveToDevice from '@/modules/save-to-device';
